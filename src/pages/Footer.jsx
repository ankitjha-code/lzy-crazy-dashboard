  ...
  const fetchFooterData = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:4000/api/footer/test",
        { userId: user._id }
      );
      ...
      if (fileInputRef.current && fileInputRef.current.files[0]) {
        ...
        try {
          ...
          const fileResponse = await axios.post(
            "http://localhost:4000/api/footer/upload",
            fileData,
            {
  ...
  return (
    <div className="w-full max-w-[1550px] mx-auto mb-8">
      ...
      <form
        ...
        className="card-container bg-white rounded-xl border border-gray-200 shadow-md px-1 py-6 md:p-6 mb-8"
      >
        <div className="flex flex-col md:flex-row gap-5">
          <div className="w-full md:w-1/2 bg-white rounded-xl border border-gray-200 shadow-md p-4 text-white">
            <label className="text-black text-xl sm:text-2xl">Logo</label>
                ...
                {socialIcons.map((icon) => (
                  ...
                  >
                    <div className="relative text-black w-12 h-12 flex items-center justify-center border border-gray-300 rounded-md bg-gray-50">
                      {icon.platform?.id === "facebook" && (
                        <Facebook size={24} />
                      )}
                      {icon.platform?.id === "twitter" && <Twitter size={24} />}
                      {icon.platform?.id === "instagram" && (
                        <Instagram size={24} />
                      )}
                      {icon.platform?.id === "linkedin" && (
                        <Linkedin size={24} />
                      )}
                    ...
                    <div className="flex-1">
                      ...
                      <input
                        ...
                        value={icon.link}
                        onChange={(e) =>
                          handleLinkChange(icon.id, e.target.value)
                        }
                      />
                    </div>
                  </div>
        ...
        <div className="pt-9 text-center">
          ...
          <button
            type="submit"
            disabled={loading}
            className={`bg-blue-600 text-white px-8 py-2 rounded-md text-lg hover:bg-blue-700 transition ${
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete this footer? This action cannot be
              undone.
            </p>
