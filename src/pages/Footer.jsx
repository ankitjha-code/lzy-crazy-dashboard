...
const Footer = () => {
  ...
  const fetchFooterData = async () => {
    ...
    if (!isAuthenticated) {
      toast.error('You need to be logged in to access footer data');
    ...
    try {
      // Send user ID in the request body as required by backend
      ...
      const response = await axios.post('/footer/get', { userId: user?._id });
  ...
  const handleDeleteFooter = async () => {
    setIsDeleting(true);
    try {
      // Use the authenticated route instead of the test route
      const response = await axios.delete('/footer');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add userId to form data - this is required by the backend
      if (user?._id) {
        console.log('Adding user ID to form data:', user._id);
      ...
      formData.append('recentPosts', JSON.stringify(recentPosts));
      formData.append('dayTimes', JSON.stringify(dayTimes));
      
      // Send everything in a single request to the create footer endpoint
      const response = await axios.post('/footer/create', formData, {
      ...
      if (err.message.includes('Network Error') || err.message.includes('connection')) {
        try {
          ...
          const fallbackResponse = await axios.post('/footer/create', simpleData);
